...
\title{Two Ways to Combine Numerical and Classification Objectives in Time-Series Foundation Models}
...
classification-style training and numerical-distance objectives emphasize
different forecasting behavior.  The original CE objective remains strongest for
WQL, while distance-aware objectives such as bin-W1, CRPS-style loss, and
ordinal CE improve or balance other aspects of the forecast.  This suggests
that the two objective families are not simply substitutes: categorical
probability modeling and numerical distance information provide complementary
signals during fine-tuning.
...
head makes a classification-style adaptation feasible, and CE64 with LoRA
achieves the best MASE among our TimesFM variants.  This result is important
because the benefit is not limited to probabilistic bin prediction; the
classification formulation also helps point-to-point forecasting after
fine-tuning.  In other words, combining the strengths of classification
objectives and numerical forecasting backbones can expose capabilities that are
not visible under the original output formulation alone.
